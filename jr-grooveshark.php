<?php
/*
Plugin Name: JR Grooveshark
Plugin URI: http://www.jakeruston.co.uk/2010/05/wordpress-plugin-jr-grooveshark/
...
add_action('admin_menu', 'jr_Grooveshark_add_pages');
...
if (!function_exists("jr_Grooveshark_refresh")) {
function jr_Grooveshark_refresh() {
update_option("jr_submitted_Grooveshark", "0");
}
}

register_activation_hook(__FILE__,'grooveshark_choice');

function grooveshark_choice () {
if (get_option("jr_Grooveshark_links_choice")=="") {

if (_iscurlinstalled()) {
$pname="jr_Grooveshark";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_Grooveshark", "1");
wp_schedule_single_event(time()+172800, 'jr_Grooveshark_refresh'); 
} else {
...
update_option("jr_Grooveshark_links_choice", $content);
}
}

if (get_option("jr_grooveshark_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_grooveshark_link_personal", $content);
}
}

// action function for above hook
function jr_Grooveshark_add_pages() {
    add_options_page('JR Grooveshark', 'JR Grooveshark', 'administrator', 'jr_Grooveshark', 'jr_Grooveshark_options_page');
}

// jr_Grooveshark_options_page() displays the page content for the Test Options submenu
function jr_Grooveshark_options_page() {
...
<?php
    ...
    echo "<h2>" . __( 'JR Grooveshark Plugin Options', 'mt_trans_domain' ) . "</h2>";
$blog_url_feedback=get_bloginfo('url');
...
<p><?php _e("Number of links to Show:", 'mt_trans_domain' ); ?> 
...
if ($widget_title=="") {
$widget_title="Recent Grooveshark Plays";
}
...
foreach ($items as $item) :
    $Groovesharkdisp .= '<li><font color="#000000" size="2"><a rel="nofollow" href="'.$item['link'].'">'.$item['title'].'</a></font></li><br />';
...
$linkper=utf8_decode(get_option('jr_grooveshark_link_personal'));

if (get_option("jr_grooveshark_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_grooveshark_links_choice'));
...
update_option("jr_grooveshark_links_choice", $new);
update_option("jr_grooveshark_link_newcheck", "444");
}

if (get_option("jr_submitted_Grooveshark")=="0") {
$pname="jr_Grooveshark";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_Grooveshark", "1");
...
wp_schedule_single_event(time()+172800, 'jr_Grooveshark_refresh'); 
} else if (get_option("jr_submitted_Grooveshark")=="") {
$pname="jr_Grooveshark";
$url=get_bloginfo('url');
$current=get_option("jr_Grooveshark_links_choice");
...
$Groovesharkdisp .= "<p style='font-size:x-small'>Grooveshark Plugin created by ".$linkper." - ".stripslashes(get_option('jr_Grooveshark_links_choice'))."</p>";
}
...
register_sidebar_widget("JR Grooveshark Recent Plays", "show_Grooveshark_user");
}
